...
fanart = xbmc.translatePath(os.path.join('special://home/addons/plugin.program.t2kadvancedfix', 'fanart.jpg'))
iconart = xbmc.translatePath(os.path.join('special://home/addons/plugin.program.t2kadvancedfix', 'icon.png'))
SETTINGS_PATH = os.path.join(xbmc.translatePath('special://profile/addon_data/plugin.program.t2kadvancedfix'), 'settings.xml')
...
def MENU(name):
    ...
    addDirPlayable("Write XML File to temporary location", 'url',500, '','Play it safe. Write your advanced settings to userdata/plugin.program.t2kadvancedfix/XML_FILES/ directory',editfile,'')
...
def audiovideo(name):
    ...
    for d in dirlist:
        ...
        if currentsetting == 'DISABLED':
           d = "[COLOR blue]%s[/COLOR] (%s)" % (splitd[0], currentsetting)
        else:
            d = "[COLOR gold]%s[/COLOR] (%s)" % (splitd[0], currentsetting) 
...
def networkmenu(name):
    addDir("Cache", 'url',302, '','blank','Network client settings')
    name = "root"
...
def cache(name):
	...
	'Kodi 17 & 18  Settings <>text<>Use these settings if you are using kodi 17 or later<>',
...
def write_xml(name,dir_path):
    ...
    for s in settings:
        try:
            ...
            pass
    if count > 0:
        write_to_file(dir_path, '<!-- Created using Top Tutorials Buffer Fix addon -->\n<advancedsettings>\n', False)
        settinglist = ['video&lt;&gt;','audio&lt;&gt;','cache&lt;&gt;','edl&lt;&gt;','pvr&lt;&gt;','epg&lt;&gt;','samba&lt;&gt;','videolibrary&lt;&gt;','videodatabase&lt;&gt;','musicdatabase&lt;&gt;','musiclibrary&lt;&gt;','karaoke&lt;&gt;','tuxbox&lt;&gt;','pathsubstitution&lt;&gt;','masterlock&lt;&gt;']
        ...
        for s in settings:
            ...
            if value != 'DISABLED' and 'id="viewtype"' not in s:
                ...
                if rootid not in str(settinglist):#!= 'video' and rootid != 'audio' and rootid != 'cache' and rootid != 'edl' and rootid != 'pvr' and rootid != 'samba' and rootid != 'epg'
...
def viewxml(name):
    ...
    TextBoxes("[B][COLOR gold]Your advancedsettings.xml file[/B][/COLOR]",msg)

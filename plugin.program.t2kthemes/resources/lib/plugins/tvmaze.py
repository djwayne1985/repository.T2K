...
from resources.lib.util.xml import JenItem, JenList, display_list, display_list2
...
def fetch_from_db2(url):
    koding.reset_db()
    url2 = str(url.replace("://","").replace(".","").replace("%20","").replace("/","").replace("_","").replace("-","").replace("'","").replace("|","").replace(" ",""))
